...
public class Banco {
    ...
    private static Conta buscarConta(String identificador, String senha) {
        for (int i = 0; i < numContas; i++) {
            if (contas[i].getId().equals(identificador) && contas[i].validaAcesso(identificador,senha)) {
                return contas[i];
            }
        }
        return null;
    ...
    private static void consultarSaldo() {
        System.out.print("Identificador da contas: ");
    ...
    private static void gravarContasPoupanca() {
        try (PrintWriter pw = new PrintWriter(new FileWriter("contasPoupanca.txt"))) {
            for (Conta conta : contas) {
                if (conta instanceof ContaPoupanca) {
                    pw.println(conta.getId() + ";" + conta.getTitular() + ";" + conta.getSaldo());
                }
            }
        } catch (IOException e) {
            System.out.println("Ocorreu um erro ao gravar o arquivo de contas poupança.");
            e.printStackTrace();
        }
    }

    private static void gravarContasCorrente() {
        try (PrintWriter pw = new PrintWriter(new FileWriter("contasCorrente.txt"))) {
            for (Conta conta : contas) {
                if (conta instanceof ContaCorrente) {
                    ContaCorrente cc = (ContaCorrente) conta;
                    pw.println(cc.getId() + ";" + cc.getTitular() + ";" + cc.getSaldo() + ";" + cc.verificaSaldo());
                }
            }
        } catch (IOException e) {
            System.out.println("Ocorreu um erro ao gravar o arquivo de contas corrente.");
            e.printStackTrace();
        }
    }
}
